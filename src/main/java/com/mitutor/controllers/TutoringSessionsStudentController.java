...
public class TutoringSessionsStudentController {
	...
	public ResponseEntity<TutoringSessionStudentResponse> addStudentSession(
		...
		try {
			...
			TutoringSession tutoringSessionFound = tutoringSessionService
					.findById(createTutoringSessionsStudent.getTutoring_session_id()).get();
			...
			TutoringSessionsStudent newTutoringSessionStudent = tutoringSessionStudentService
					.save(tutoringSessionStudent);
	...
	public ResponseEntity<List<TutoringSessionResponse>> getTutoringSessionsByUser(
			@PathVariable("userId") Integer userId){
    	
    	List<TutoringSession> allTutoringSession =  tutoringSessionStudentService.findSessionsByUserId(userId);
    	List<TutoringSessionResponse> response  = new ArrayList<TutoringSessionResponse>();
    	
    	for (int i = 0; i< allTutoringSession.size(); ++i) {
    		TutoringSessionResponse newTutoringResponse =  new TutoringSessionResponse();
    		newTutoringResponse.setPlace(allTutoringSession.get(i).getPlace());
    		newTutoringResponse.setStartTime(allTutoringSession.get(i).getStartTime());
    		newTutoringResponse.setEndTime(allTutoringSession.get(i).getEndTime());
    		response.add(newTutoringResponse);
    	}
    	return new ResponseEntity<>(response,HttpStatus.OK);
    }
}
